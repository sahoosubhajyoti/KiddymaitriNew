...
export default function Clock({ time }: { time: string }) {
 ...
 return (
  <div className="w-40 h-40 bg-gray-100 border-4 border-gray-700 rounded-full relative shadow-inner">
   {/* Clock Face Numbers */}
   <div className={`${numStyle} top-2 left-1/2 -translate-x-1/2`}>||</div>
   <div className={`${numStyle} top-4 right-10`}>.</div>
   <div className={`${numStyle} top-10 right-4`}>.</div>
   <div className={`${numStyle} right-2 top-1/2 -translate-y-1/2`}>.</div>
   <div className={`${numStyle} bottom-10 right-4`}>.</div>
   <div className={`${numStyle} bottom-4 right-10`}>.</div>
   <div className={`${numStyle} bottom-2 left-1/2 -translate-x-1/2`}>|</div>
   <div className={`${numStyle} bottom-4 left-10`}>.</div>
   <div className={`${numStyle} bottom-10 left-4`}>.</div>
   <div className={`${numStyle} left-2 top-1/2 -translate-y-1/2`}>.</div>
   <div className={`${numStyle} top-10 left-4`}>.</div>
   <div className={`${numStyle} top-4 left-10`}>.</div>
